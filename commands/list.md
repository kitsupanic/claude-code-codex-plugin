...
- `failed(record-write-refused)` — a write the run depends on could not be made to the record: the sight label the delivery gate reads, or the codex pids a cancel has to aim at. The record lock could not be taken (an impostor, a stalled holder) or the record had gone corrupt. If codex had already been spawned it was killed first — a run nobody can cancel is not allowed to continue — and the job says on stderr and in its `run.log` whether that kill was verified.
