...
argument-hint: '<brief-file-or-inline-text> [--role <stem>] [--cd <dir>] [--model <m>] [--effort <e>] [--write] [--force] [--watch] [--allow-unproven-sight]'
...
2. Run (passing through any `--role`, `--cd`, `--model`, `--effort`, `--write`, `--force`, `--watch`, `--allow-unproven-sight` flags the user gave — `--watch` also opens a console window that follows the job and shouts when it finishes, which is for the user, not for you; `--allow-unproven-sight` is passed through **only when the user typed it**, never added on your own):
...
If the dispatch is refused, relay the refusal as-is rather than retrying around it. A same-role job that is still alive, a lost race for the role, a claim taken over mid-startup, or a previous job whose kill could not be verified are all deliberate refusals — the failure they prevent is two billing Codex processes at once.

A job that ends as `failed / sight-unproven` is the other deliberate refusal: nothing proved Codex could read files where it was pointed, so it never ran and nothing was billed. Relay it with the cure it names — usually `npm install -g @openai/codex`, or a `--cd` pointed at the directory that actually has to be read. **Do not re-dispatch with `--allow-unproven-sight` to get past it.** That flag exists so a user can knowingly accept an answer nothing vouched for; it is theirs to ask for, and if it seems like the right move, say so and let them decide.
