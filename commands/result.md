...
- **Not delivered** — anything else: running, stale, failed, killed, kill-pending, kill-failed, unknown (a record whose state is not one this release writes, which is live and unvouched rather than finished). Exits nonzero naming the state, the runtime, and the `out:` path. If an answer file exists anyway the refusal says so and still refuses, because nothing has vouched for how that run ended. Do not fetch that file for the user unless they ask for it by name.
