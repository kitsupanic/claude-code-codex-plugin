...
| **Tests** | 8 test files, CI on every pull request | 171 tests — fake-codex lifecycle drills, the deliverability matrix, sight-gate and kill-verification drills, path-escape canaries, a concurrency race and a fenced-claim takeover, plus an opt-in live smoke |
...
Current release: **0.8.6** — a five-patch arc on the record lock (0.8.2–0.8.5,
each release a second opinion on the one before it: an atomic rename so no writer
sees a lock without a holder, every destructive step proving it is destroying
what it condemned, the room behind that closed by refusing to acquire past a live
stranded lock, and every write whose loss would strand a record checked and said
out loud) — and then 0.8.6 turned the same adversarial lens on the KILL seam:
process identity under pid reuse. The kill decision stays fail-open so a slow
process table never leaves real codex billing, but what gets recorded as a
target now fails closed, a verified-dead pid is marked spent, a failed
start-time query no longer poisons the cache, and the launch phase can never
rewind. Blow-by-blow in [docs/CHANGELOG.md](docs/CHANGELOG.md).

0.8.1 — a dual review of 0.8.0, whose lead finding is a
...
— gone meaning genuinely absent, or holder content no release of this runtime
writes, which is corruption evidence rather than a live acquirer.
...
```
node --test                            # everything: 171 tests (bare form — see below)
