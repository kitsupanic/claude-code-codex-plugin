...
Current release: **0.6.0** — the post-0.5.0 review, fixed: the POSIX sight probe
quotes for `sh`, a reissued pid is identified by its start time and never fired
at, the exit-time record write is preconditioned, and `reaped.pids` is written
atomically. 0.5.0 records remain deliverable. Full history, including the
